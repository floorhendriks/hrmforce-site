 ...
 *   HSF_PDF_LANGS=nl,en      talen om te genereren (standaard nl,en; kan ook de,fr,es,ro)
...
// De niet-Nederlandse talen gebruiken de Engelse padsegmenten en de Engelse slug.
const SEG = {
  nl: { jobs: 'functies', full: 'volledig', prefix: '' },
  en: { jobs: 'jobs', full: 'full', prefix: '/en' },
  de: { jobs: 'jobs', full: 'full', prefix: '/de' },
  fr: { jobs: 'jobs', full: 'full', prefix: '/fr' },
  es: { jobs: 'jobs', full: 'full', prefix: '/es' },
  ro: { jobs: 'jobs', full: 'full', prefix: '/ro' },
};
/** Slug per taal: alleen Nederlands heeft eigen slugs, de rest volgt Engels. */
const slugOf = (f, lang) => (lang === 'nl' ? f.slug.nl : f.slug.en);
...
const PAGE_LABEL = { nl: 'pagina', en: 'page', de: 'Seite', fr: 'page', es: 'página', ro: 'pagina' };

const footer = (lang) => `
  <div style="width:100%;font:8pt Arial;color:#5A6470;padding:0 14mm;display:flex;justify-content:space-between">
    <span>hrmforce &middot; hrmforce.com &middot; Skills Framework ${meta.version}</span>
    <span>${PAGE_LABEL[lang] ?? 'page'} <span class="pageNumber"></span> / <span class="totalPages"></span></span>
  </div>`;
...
for (const lang of LANGS) {
  ...
  let list = fns.filter((f) => (ONLY ? ONLY.has(slugOf(f, lang)) : true));
  ...
  const workers = Array.from({ length: Math.max(1, CONC) }, async () => {
    ...
    while (queue.length) {
      const f = queue.shift();
      const url = `http://127.0.0.1:${PORT}${seg.prefix}/skills-framework/${seg.jobs}/${slugOf(f, lang)}/${seg.full}/`;
      try {
        await page.goto(url, { waitUntil: 'networkidle', timeout: 45000 });
        await page.emulateMedia({ media: 'print' });
        await page.pdf({
          path: join(OUT, lang, `${slugOf(f, lang)}.pdf`),
